...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
	...
	public synchronized String getStreamCtrlInfo() {
		String streamCtrlInfo;
		streamCtrlInfo = nativeGetStreamCtrlInfo(mNativePtr);
		if (!TextUtils.isEmpty(streamCtrlInfo)) {
			return streamCtrlInfo;
		} else {
			return "";
		}
	}
    ...
    public synchronized void setPreviewDisplay(final Surface surface) {
		Log.d(TAG, "nativeSetPreviewDisplay" +
				"\n    pointer: " + mNativePtr + " Surface: " + surface);
    	nativeSetPreviewDisplay(mNativePtr, surface);
    }
    ...
    public void setFrameCallback(final IFrameCallback callback, final int pixelFormat) {
    	if (mNativePtr != 0) {
			Log.d(TAG, "nativeSetFrameCallback->calback: " + callback);
...
//================================================================================
	...
	private static final native String nativeGetStreamCtrlInfo(final long id_camera);

...
//			if (DEBUG) Log.i(TAG, "unregister:");
    ...
    public List<UsbDevice> getUVCCamreaDeviceList() throws IllegalStateException {

        if (destroyed) throw new IllegalStateException("already destroyed");
        final HashMap<String, UsbDevice> deviceList = mUsbManager.getDeviceList();
        final List<UsbDevice> result = new ArrayList<>();
        for (String key : deviceList.keySet()) {
            UsbDevice usb = deviceList.get(key);
            if (usb.getDeviceClass() == 239 && usb.getDeviceSubclass() == 2) {
                String vendorName = USBVendorId.vendorName(usb.getVendorId());
                result.add(usb);
                Log.d(TAG, "Find UVCCamera : " + vendorName);
            }
        }
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
	...
	private final Runnable mDeviceCheckRunnable = new Runnable() {
		...
		public void run() {
			if (destroyed) return;
			final List<UsbDevice> devices = getUVCCamreaDeviceList();
